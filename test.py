...
def calculate_mAP():
    ...
    for file in fileListPredicted:
        ...
        for item in predictedObject:
            ...
            dictPredicted[classId].append(
                [confidence, file, [xmin, xmax, ymin, ymax]])
    ...
    for classId in range(numOfClasses):
        ...
        truePositives.append(
            np.zeros(numberOfPredictedObjectsInClass, dtype=np.float64))
        ...
        for predictedObjectIndex in range(len(dictPredicted[classId])):
            ...
            for GTObjectIndex in range(len(dictGT[classId][predictedItem[1]])):
                ...
                areaMetric = IoU(GTItem[GTObjectIndex], predictedItem[2])
    ...
    for classId in range(numOfClasses):
        ...
        for image in dictMask[classId]:
            falseNegatives[classId] += sum(
                [not x for x in dictMask[classId][image]])
        cumulativePrecision.append(
            np.divide(
                np.cumsum(truePositives[classId]),
                np.cumsum(truePositives[classId]) +
                np.cumsum(falsePositives[classId])))
        #cumulativePrecision.append(np.divide(np.cumsum(truePositives[classId]),
        #1 + np.arange(totalPredicted[classId])))

        cumulativeRecall.append(
            np.cumsum(truePositives[classId]) / totalGT[classId])
    ...
    print("Class-Name", "Total Ground Truth", "Total Predicted",
          "True Positives", "False Positives", "False Negatives",
          "Average Precision")

    for classId in range(numOfClasses):
        print(classes[classId], totalGT[classId], len(dictPredicted[classId]),
              np.sum(truePositives[classId]), np.sum(falsePositives[classId]),
              falseNegatives[classId], averagePrecision[classId])
